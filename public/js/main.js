...
document.addEventListener("click", function (e) {
  // Hỗ trợ cả class .toggle-password và .toggle-icon
  const toggleBtn = e.target.closest(".toggle-password, .toggle-icon");

  if (toggleBtn) {
    e.preventDefault();
    // Tìm container cha (hỗ trợ .password-group... hoặc lấy cha trực tiếp nếu không có class)
    const group =
      toggleBtn.closest(
        ".password-group, .password-wrapper, .form-group, .input-group",
    ...
    if (group) {
      ...
      if (input) {
        ...
        // Chuyển đổi icon (nếu là font awesome)
        const icon =
          toggleBtn.tagName === "I" ? toggleBtn : toggleBtn.querySelector("i");
        if (icon) {
